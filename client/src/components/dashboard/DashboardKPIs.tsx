...
export const DashboardKPIs = ({ total, completed, delayed, inProgress, completionRate, initiatives }: KPIProps) => {
    ...
    // Ensure we match how metrics are calculated exactly: (this is a rough guess based on typical names, but verify later)
    // Actually, to match exactly what is passed from parent:
    // Total: active initiatives
    // Completed: Entregado
    // Delayed: Retrasado
    // In Progress: not delivered and not delayed? Or "En Tiempo", "En Riesgo".
    // I'll update the filter logic to match what is usually there
    ...
    const refinedFilteredInitiatives = useMemo(() => {
        ...
        switch (selectedKPI) {
            ...
            case 'En Ejecución':
                return activeInitiatives.filter(i => i.status !== 'Entregado' && i.status !== 'Retrasado');
            case 'Riesgo / Retraso':
    ...
    return (
        <>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4">{/* Total Initiatives Card */}
                <div
                    className="bg-white dark:bg-[#1E2630] rounded-xl p-6 shadow-sm border border-gray-100 dark:border-gray-800 relative overflow-hidden group hover:shadow-md transition-all cursor-pointer"
