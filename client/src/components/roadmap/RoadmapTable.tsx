    ...
    return (
        <React.Fragment>
            <tr 
                ...
                isDragging ? "bg-blue-50 dark:bg-blue-900/20" : "hover:bg-[var(--bg-tertiary)] dark:hover:bg-[#2A3441]",
                ...
                <td className={clsx(
                    ...
                    isDragging && "bg-blue-50 dark:bg-blue-900/40"
                )}>
                    <div className="flex items-center justify-between group-hover:pr-2 transition-all">
                        <div className="flex items-center flex-1 min-w-0">
                            {(user?.role === 'admin' || user?.role === 'editor') && !isPresentationMode && (
                                <div {...listeners} className="mr-2 cursor-grab active:cursor-grabbing text-[var(--text-tertiary)] hover:text-[var(--text-primary)] p-1 rounded transition-colors hover:bg-white/10">
                ...
                <td className="px-2 py-2 text-[10px] border-r border-[var(--border-color)] text-center text-[var(--text-secondary)] dark:text-gray-200">
                    <div className="flex flex-col space-y-1">
                        {initiative.technologies?.map((t: string) => (
...
export const RoadmapTable = () => {
    ...
    const handleDragEndAction = async (event: DragEndEvent) => {
        ...
        setInitiatives((items) => {
            ...
            // Sync with backend
            syncReorder(newList.map(i => i.id));

            return newList;
        });
    };

    const syncReorder = async (ids: string[]) => {
        try {
            await fetch(`${API_URL}/api/initiatives/reorder-all`, {
                method: 'PATCH',
                headers: {
                    'Content-Type': 'application/json',
                    Authorization: `Bearer ${token}`
                },
                body: JSON.stringify({ initiativeIds: ids })
            });
        } catch (e) {
